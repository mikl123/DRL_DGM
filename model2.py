...
class Model2(nn.Module):
    ...
    def __init__(self, output_dim, hidden_dim=128):
        ...
        self.fc = nn.Sequential(
            nn.Linear(2 * output_dim + output_dim - 1, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, 2)  # outputs MU_j and log_SIGMA_j
        )
...
def compute_density(mu, sigma, x):
...
def prepare_data(model1, x, y, config):
    ...
    x_predicted_m1 = torch.cat([x_predicted_m1["mu"], x_predicted_m1["sigma"]], dim=-1)
...
def train_model2(model1, x_train, y_train, x_val, y_val, config):
    # Prepare input
